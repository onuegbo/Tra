...
class Cart(models.Model):
    ...
    status = models.CharField(max_length=32, choices=CartStatus.CHOICES, default=CartStatus.OPEN)
    created = models.DateTimeField(auto_now_add=True)
    last_status_change = models.DateTimeField(auto_now_add=True)
    user = models.ForeignKey(User, blank=True, null=True, related_name='carts', on_delete=models.CASCADE)
    email = models.EmailField(blank=True, null=True)
    token = models.UUIDField(primary_key=True, default=uuid4, editable=False)
    voucher = models.ForeignKey('discount.Voucher', null=True, related_name='+', on_delete=models.SET_NULL)
    ...
    def is_shipping_required(self):
            ...
            return any(line.is_shipping_required() for line in self.lines.all()) #link to cart lines
    ...
    def add(self, variant, quantity=1, data=None, replace=False, check_quantity=True):
            ...
            if check_quantity:
                variant.check_quantity(new_quantity) # product variant
...
class CartLine(models.Model, ItemLine):
    ...
    variant = models.ForeignKey('etrans.ProductVariant', related_name='+', on_delete=models.CASCADE)
