...
def products_visible_to_user(user):
    ...
    if user.is_authenticated and user.is_active and user.is_staff:
        ...
        return Product.objects.available_products() #see Queryset


def products_with_details(user):
    products = products_visible_to_user(user)
    products = products.prefetch_related(
        'company', 'images', 'variants__stock',
...
def handle_cart_form(request, product, create_cart=False):
    ...
    form = ProductForm(cart=cart, product=product, data=request.POST or None, discounts=request.discounts)
    return form, cart
...
def get_variant_picker_data(product, discounts=None, local_currency=None):
    ...
    variant_attributes = product.product_type.variant_attributes.all() # model fields
    ...
    for variant in variants:
        ...
        for variant_key, variant_value in variant.attributes.items():
            filter_available_variants[variant_key].append(variant_value)
...
def get_product_availability_status(product):
    ...
    are_all_variants_in_stock = all(variant.is_in_stock() for variant in product.variants.all())
